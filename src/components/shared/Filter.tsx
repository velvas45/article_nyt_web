...
import { DATA_SORTING_ARTICLE } from "../../constant";
...
const Filter = ({ value, setFilter }: Props) => {
  return (
    <Listbox value={value} onChange={setFilter}>
      <div className="relative mt-1 w-full md:w-[200px]">
        ...
        >
          <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-50">
            {DATA_SORTING_ARTICLE.map((item, idx) => (
              <Listbox.Option
                key={idx}
                className={({ active }) =>
